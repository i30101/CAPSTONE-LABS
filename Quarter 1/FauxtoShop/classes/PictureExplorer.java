...
import javax.swing.filechooser.*;//rg added
 ...
 * JTextField memory fix by Sam Schumer
 */
    ...
    */
    ...
    /** Method to handle when the user releases the mouse */
        public void mouseReleased(MouseEvent e) {
            x_Lower_Right = e.getX();
            y_Lower_Right = e.getY();
    ...
    private class Listener_Open implements ActionListener {
        public void actionPerformed(ActionEvent e) {
            ...
            JFileChooser chooser = new JFileChooser(new File(start.toString() + "/images"));
    ...
    */
    ...
    private class Listener_ModRed_TextField implements ActionListener {
        public void actionPerformed (ActionEvent e) {
            ...
            updatePicture();
            tara2.removeActionListener(this);
        }
    }		
    ...
    private class Listener_ColorSplash implements ActionListener {
        public void actionPerformed(ActionEvent e) { 
            ((Picture)picture).colorSplash(colorPanel.getBackground());
            updatePicture();
        }
    }
    ...
    private class Listener_Pixelate implements ActionListener {	      
        public void actionPerformed(ActionEvent e) {
            tara2.addActionListener(new Listener_Pixelate_TextField());
            tara.setText("Enter pixel block size (4,8,16,etc): ");
        }
    }

    private class Listener_Pixelate_TextField implements ActionListener {
        public void actionPerformed (ActionEvent e) {
            int blockSize = Integer.parseInt(tara2.getText());
    ...
    private class Listener_Encode implements ActionListener {
        public void actionPerformed(ActionEvent e) {
            File start = new File("").getAbsoluteFile().getParentFile();
            //Alex Friedman - use default directory
            JFileChooser chooser = new JFileChooser(new File(start.toString() + "/images"));
            ...
            if(returnVal == JFileChooser.APPROVE_OPTION) 
            {
                ...
                Picture messagePict = new Picture(chooser.getSelectedFile().getAbsolutePath());
                ((Picture)picture).encode(messagePict);
            }
            updatePicture();
        }
    }
